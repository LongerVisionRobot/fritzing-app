...
QString PaletteItemBase::retrieveSvg(ViewLayer::ViewLayerID viewLayerID, QHash<QString, QString> & svgHash, bool blackOnly, double dpi, double & factor)
{
    ...
    QString svg = svgHash.value(path + xmlName + QString(QChar(ushort(m_viewLayerPlacement))), "");
	...
	svgHash.insert(path + xmlName + QString(QChar(ushort(m_viewLayerPlacement))), svg);
	return svg;
}
